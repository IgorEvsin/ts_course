...
\definecolor{codecol}{RGB}{225, 225, 235}    % Цвет фона
\definecolor{titlecodecol}{RGB}{200, 200, 220} % Цвет заголовка
...
%- Абстракт в начале  (просто текст) % поискать abstract дфеуч
%- Краткие примеры  сделали серым, надо попрозрачнее .done
%-куски кода (это есть из python_higlghtligting)
%- Небольшие бизнес-кейсы \defn
%- Дальнейшее чтение просто список
%- Вопросы на закрепление /proof .done
%- Практические вопросы?
% - Определения \def . done
%TODO: Сделать краткие примеры попрозрачнее .done
...
%  % Стилевые настройки:
%  enhanced,
%  arc=2pt, % Скругление углов
%  left=6mm, % Отступ слева (для номеров строк)
%  colback=codecol, % Цвет фона
%  colframe=titlecodecol, % Цвет рамки
%  coltitle=black, % Цвет текста заголовка
%  fonttitle=\bfseries, % Шрифт заголовка
%  title=Пример~\thetcbcounter: #2, % Формат заголовка
%  #1, % Дополнительные параметры
%  listing only, % Только код, без обычного текста
%  %listing options={style=mypython}, % Применяем стиль кода
%  breakable % Разрешаем разрыв блока на страницы
%}

%% Упрощенная команда для вставки (опционально)
%\NewDocumentCommand{\pythoncode}{O{} m m}{%
%  \begin{pythonexample}[#1]{#2}
%    #3
%  \end{pythonexample}%
%}
...
%% Assumption
%% ============================
\newtcbtheorem[use counter from=mydefinition]{myassumption}{Еще какой-то блок}
{
  enhanced,
  frame hidden,
  titlerule=0mm,
  ...
  opacityback=0.7, % Параметр, который отвечает за прозрачность
...
\newtcbtheorem[use counter from=mydefinition]{myexample}{Краткие примеры}
{
  enhanced,
  frame hidden,
  titlerule=0mm,
...
\NewDocumentCommand{\que}{m+m}{
  \begin{questions}
    {\noindent{\it \textbf{\large{Вопросы для самопроверки} : #1 }}}\\
    \begin{enumerate}
        #2
    \end{enumerate}
  \end{questions}
...
\newenvironment{pr_questions}{
  \tcolorbox[blanker,breakable,left=5mm,parbox=false,
    before upper={\parindent15pt},
    after skip=10pt,
  borderline west={1mm}{0pt}{green!70!yellow}] % Салатовый цвет
}{
  \endtcolorbox % Убраны символы квадрата
}

\NewDocumentCommand{\prque}{m+m}{
  \begin{questions}
    {\noindent{\it \textbf{\large{Практические вопросы} : #1 }}}\\
    \begin{enumerate}
        #2
    \end{enumerate}
  \end{questions}
...
% \begin{tcolorbox}[colback=yellow!20!white,colframe=yellow!15!black,title=
% \textbf{Définition \thechapter \ $\blacklozenge$ \thedefo \ |
% #1}]}{\end{tcolorbox}}

% %-------    THEOREME     -------
% \newcounter{th}[chapter]
% \newenvironment{thm}[1]{\refstepcounter{th}
% \begin{tcolorbox}[colback=mycolor!10,colframe=mycolor!10!black!80,title=\textbf{
% Théorème \thechapter \ $\blacklozenge$ \theth \ | #1}]}{\end{tcolorbox}}

% %-------    PROPOSITION     -------
% \newcounter{prop}[chapter]
% \newenvironment{propt}[1]{\refstepcounter{prop}
% \begin{tcolorbox}[colback=mycolor!5,colframe=mycolor!10!linkscolor!80
% ,title=\textbf{ Proposition \thechapter \ $\blacklozenge$ \theprop
% \ | #1}]}{\end{tcolorbox}}

% %-------    COROLLAIRE    -------
% \newcounter{cor}[chapter]
% \newenvironment{corr}[1]{\refstepcounter{cor}
% \begin{tcolorbox}[colback=mycolor!2,colframe=mycolor!10!linkscolor!40
% ,title= Corolaire \thechapter \ $\blacklozenge$ \thecor \ |
% #1]}{\end{tcolorbox}}

% %-------    LEMME      -------
% \newcounter{lem}[chapter]
% \newenvironment{lemme}[1]{\refstepcounter{lem}
% \begin{tcolorbox}[colback=mycolor!10!blue!2,colframe=mycolor!50!blue!30,title=\textbf{Lemme
% \thechapter \ $\blacklozenge$ \thelem \ | #1}]}{\end{tcolorbox}}
...
% {\textbf{Méthode \thechapter \ $\blacklozenge$ \themet \ | #1} }
% \color{black} \\ } {\end{tcolorbox}}
