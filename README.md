...
|------|------------|-----------|
| **0. Временные ряды как реализация случайного процесса** | <ul><li>Базовые концепции и определения</li><li>Основные задачи анализа временных рядов </li><li>Что такое качественный прогноз<li>Метрики качества точечного прогноза</li></ul> | [Adeshere: Введение в TS](https://habr.com/ru/articles/542638/) |
...
## СТРУКТУРА КУРСА МОЖЕТ МЕНЯТЬСЯ!

Если мы столкнемся с важными материалами, которые не будут адекватно влезать в структуру, мы изменим ее.

Пока что непонятно, где в существующей структуре расположить:
- ARIMA/ARIFMA/SARIMAX/GARCH
- Бутстрап моделей
- Автоэнкодеры рядов и вообще способы их симуляции
- Классификация рядов
